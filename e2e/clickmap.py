...
_KEY_LOGIC = """
  ...
  // 內容由資料產生的元素（常用快選 chip）：用 class，不要用文字。
  // 文字是廠商名，每次隨機資料都不同，用文字會變成永遠點不完的新 key。
  if (cls === '.chip') return e.tagName.toLowerCase() + cls;
...
SKIP = {
    'button「已記錄 ✓」': '與「送出這一筆」是同一顆，送出成功後文字才變成這樣',
    'button「已完成 ✓」': '同上，Busy.run 成功後的暫時字樣',
    'button「已下載 ✓」': '與「下載 Excel」是同一顆',
    'button「已鎖定 ✓」': '與「鎖定這個月」是同一顆',
    'button「已解鎖 ✓」': '與「解除鎖定」是同一顆',
    'button「解除鎖定」': '與「鎖定這個月」是同一顆，鎖定後才變成這個字',
    'input.': '檔案挑選欄（收據照片）——本機測試不真的上傳檔案',
}
